...
void GameLogic::update(const float &_timestep)
{
    ...
    // 4: store old car positions
    {
        ...
        mOldCarData.push_back(OldCarData{timestamp, {getCarCenterPosition(PlayerID::P1), getCarCenterPosition(PlayerID::P2)}, {getCarLinearVelocity(PlayerID::P1), getCarLinearVelocity(PlayerID::P2)}});
    }
    // reset input
    mUserInput->reset();
}
